...
public class FileUtil {
	...
	public static PrintStream createPrintStream(String fileName) {
		try {
			...
			throw ApplicationException.instance(AppExceptionIdentifier.TECHNICAL_EXCEPTION, e).details("Error creating PrintStream for " + fileName);
		}
	...
	public static void writeLine(BufferedWriter writer, String s) {
		try {
			...
			throw ApplicationException.instance(AppExceptionIdentifier.TECHNICAL_EXCEPTION, e).details("IO exception in writeLine");
		}
	}
	...
	public static void writeToResource(Class<?> caller, String resource, String s, boolean append) {
		String fileName;
		try {
			fileName = caller.getProtectionDomain().getCodeSource().getLocation().toURI().getPath().replace("target/classes/", resource)
					.replace("target/test-classes", resource);
		} catch (URISyntaxException e) {
			throw ApplicationException.instance(AppExceptionIdentifier.TECHNICAL_EXCEPTION, e).details("Invalid resource name specified" + resource);
		}
	...
	public static void writeToFile(String fileName, String s, boolean append) {
		try {
			...
			throw ApplicationException.instance(AppExceptionIdentifier.TECHNICAL_EXCEPTION, e).details("Exception writing to file:" + fileName);
		}
	}
	...
	public static List<String> readLines(String fileName, boolean ignoreComment) {
		...
		try {
			...
			throw ApplicationException.instance(AppExceptionIdentifier.TECHNICAL_EXCEPTION, e).details("Error reading file" + fileName);
		} finally {
			try {
				if (reader != null) {
					reader.close();
