...
public class ObjectDiff {
	...
	/**
	 * Compare the specified numeric/string attributes of the 2 objects , using the
	 * given approximation
	 *
	 * @return <code>true</code> if the values of all attributes for both objects
	 *         match, <code>false</code> otherwise
	 * @throws Exception if any of the specified attributes do not exist in the
	 *                   given Object or is not a numeric type
	 */
		...
		for (String attribute : attributes) {
			...
			if (val1 instanceof Number) {
				...
				if (diff.doubleValue() > approximation) {
					log.trace(attribute + " mismatch:" + nf.format(val1) + ":" + nf.format(val2) + " for " + object1
							+ " and " + object2);
	...
	public String toString() {
		...
		for (FieldDiff f : fieldDiffs) {
			sb.append("[").append(f.getFieldName()).append("(expected:").append(asString(f.getVal1()))
					.append(":actual:").append(asString(f.getVal2())).append("]");
		}
		return sb.toString();
	}
