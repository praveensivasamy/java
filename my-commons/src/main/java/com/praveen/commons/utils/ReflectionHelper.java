...
public class ReflectionHelper {
	...
	/**
	 * Cache the already introspected {@link Class#getFields()} The cache is stored
	 * per thread to insure thread-safety without synchronization oveerhead
	 */
	...
	/**
	 * Create a new instance of the given class name for the given constructor
	 * arguments
	 *
	...
	public static Object createObject(String className, Object... arguments) {
		try {
			...
			throw ApplicationException.instance(AppExceptionIdentifier.TECHNICAL_EXCEPTION, e)
					.details("Invalid class name specified: " + className);
		}
	}
	...
	public static <T> T createObject(Class<?> klass, Object... args) {
		...
		throw ApplicationException.instance(AppExceptionIdentifier.TECHNICAL_EXCEPTION)
				.details("Cannot instantiate " + klass.getSimpleName() + " with arguments: " + args
						+ ". No constructor with specified arguments exists");
	}
	 ...
	 * @param args
	 * @return true if the specified args are of the same number and type as the
	 *         given parameterTypes
	 */
	 ...
	 * <p>
	 * The attribute must comply with the JavaBean convention, i.e. the public
	 * [type] get[AttributeName]() method must exist
	 *
	 * @return the
	 */
	public static Object getPropertyValue(String attributeName, Object o) {
		...
		if (descriptor == null) {
			throw new RuntimeException(
					"No attribue with name " + attributeName + " found in " + o.getClass().getSimpleName());
		}
		if (descriptor.getReadMethod() == null) {
			throw new RuntimeException(
					"No read method for " + o.getClass().getSimpleName() + "." + descriptor.getName());
		}
		try {
	...
	public static Object setPropertyValue(Object o, String attributeName, String value) {
		...
		if (descriptor.getWriteMethod() == null) {
			throw new RuntimeException(
					"No write method for " + o.getClass().getSimpleName() + "." + descriptor.getName());
		}
		try {
			Object convertedValue = convert(descriptor.getPropertyType(), value);
			return descriptor.getWriteMethod().invoke(o, convertedValue);
		} catch (Exception e) {
			throw ApplicationException.instance(AppExceptionIdentifier.TECHNICAL_EXCEPTION, e).details(
					"Reflection error for Object " + o + ", attributeName " + attributeName + " and value " + value);
		}
	}
	...
	/**
	 * Get the value of the specified attribute for the given object using
	 * {@link Field} level reflection
	 * <p>
	 * No Java Bean convention required
	 *
	 * @param fieldName the name of the field. Can be a composite name, e.g. on a
	 *                  Leg instance: racePosition.id.posCode
	 */
	 ...
	 * <p>
	 * Sets the field to {@link Field#setAccessible(boolean)} to allow getting its
	 * value by reflection
	 *
	 * @param fieldName
	 * @param class1
	...
	private static void cacheFields(Class<? extends Object> klass) {
		try {
			...
			while (tmp.getSuperclass() != null) {
				fields = ArrayUtils.addAll(fields, tmp.getSuperclass().getDeclaredFields());
	...
	/**
	 * @return the array containing the {@link Column#name()} annotation values for
	 *         given the entity type and attributes
	 */
	public static String[] getColumnsNames(Class<?> entityType, String[] atrributes) {
		...
		try {
			for (int i = 0; i < atrributes.length; i++) {
				...
				if (column == null) {
					throw ApplicationException.instance(AppExceptionIdentifier.TECHNICAL_EXCEPTION)
							.details("No @Column annotation found for attribute " + atrributes[i] + " of class "
	 ...
	 * @return the table name for the given entity type, ie the value of the
	 *         {@link Table#name()} annotation
	 */
	 ...
	 * @param value     the string representation, it is converted to proper data
	 *                  type value
	 */
	public static void setValue(Object obj, String attribute, String value) {
		...
		if (field == null) {
			throw ApplicationException.instance(AppExceptionIdentifier.TECHNICAL_EXCEPTION).details(
					"No field with name '" + attribute + "' exists in class " + obj.getClass().getSimpleName());
		}
		field.setAccessible(true);
	...
	/**
	 * Convert the specified String representation to an instance of the specified
	 * type
	 */
	private static Object convert(Class<?> type, String value) {
		if (type == String.class) {
			...
			throw ApplicationException.instance(AppExceptionIdentifier.TECHNICAL_EXCEPTION)
					.details("Unsupported attribute type: " + type.getName());
		}
	}
	...
	public static Class<?> getClass(String className) {
		try {
			...
			throw ApplicationException.instance(AppExceptionIdentifier.TECHNICAL_EXCEPTION)
					.details("Invalid class name: " + className);
		}
	}
	...
	public static <T extends Annotation> T getAnnotation(Class<T> annotationClass, Class<?> klass) {
		...
		if (ann == null) {
			throw ApplicationException.instance(AppExceptionIdentifier.TECHNICAL_EXCEPTION).details(
					"Missing annotation: @" + annotationClass.getSimpleName() + " in class: " + klass.getSimpleName());
		}
		return ann;
	}
		...
		if ((fromFields != null) && (tooFields != null)) {
			for (Field tooF : tooFields) {
				try {
					...
					throw ApplicationException.instance(AppExceptionIdentifier.TECHNICAL_EXCEPTION, e)
							.details("Error in cloning object");
				}
			}
