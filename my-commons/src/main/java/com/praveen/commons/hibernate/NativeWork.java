...
/**
 * Default hibernate {@link Work} implementation for calling db functions with
			...
			for (Object argument : arguments) {
				if (argument == null) {
					...
					throw ApplicationException.instance(AppExceptionIdentifier.TECHNICAL_EXCEPTION)
							.details("Unsuported IN argument type for argument: " + argument + " at position "
									+ (paramIndex - 1) + " and function " + name);
				}
				paramIndex++;
			}
			call.execute();
